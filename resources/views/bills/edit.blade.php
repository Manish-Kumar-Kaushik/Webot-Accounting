...
<div class="max-w-5xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-slate-900 tracking-tight">Edit Vendor Bill #{{ $bill->bill_number }}</h1>
            <p class="text-xs text-slate-500">Update purchase invoice details, line items, expense category and taxes</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('bills.show', $bill->id) }}" class="text-xs text-slate-500 hover:text-slate-800">&larr; Back to Bill</a>
        </div>
    </div>
    ...
    <form action="{{ route('bills.update', $bill->id) }}" method="POST" id="billForm" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="glass-card p-6 rounded-2xl border border-slate-200 shadow-sm bg-white space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <select name="vendor_id" required class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-slate-900 text-xs focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 shadow-sm">
                        ...
                        @foreach($vendors as $v)
                            <option value="{{ $v->id }}" {{ (old('vendor_id', $bill->vendor_id) == $v->id) ? 'selected' : '' }}>
                                {{ $v->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="bill_number" value="{{ old('bill_number', $bill->bill_number) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="order_number" value="{{ old('order_number', $bill->order_number) }}" placeholder="e.g. VEN-INV-541"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 pt-3 border-t border-slate-100">
                <div>
                    ...
                    <input type="date" name="bill_date" value="{{ old('bill_date', date('Y-m-d', strtotime($bill->bill_date))) }}" required
                        class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-slate-900 text-xs focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 shadow-sm">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2">Due Date *</label>
                    <input type="date" name="due_date" value="{{ old('due_date', date('Y-m-d', strtotime($bill->due_date))) }}" required
                        class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-slate-900 text-xs focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 shadow-sm">
                </div>
                <div>
                    ...
                    <select name="category_id" required class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-slate-900 text-xs focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 shadow-sm">
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ (old('category_id', $bill->category_id) == $cat->id) ? 'selected' : '' }}>
                                {{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs" id="itemsTable">
                    <thead class="bg-slate-50 text-slate-600 font-semibold border-b border-slate-200">
                        <tr>
                            <th class="py-2.5 px-3 min-w-[280px]">Item / Expense Service</th>
                            <th class="py-2.5 px-3 w-24">Qty</th>
                            <th class="py-2.5 px-3 w-32">Price ({{ $currencySymbol }})</th>
                            <th class="py-2.5 px-3 w-32">Tax (GST/VAT)</th>
                                        ...
                                        <select onchange="onCatalogItemSelect(this)" class="catalog-select w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-slate-700 text-xs font-medium focus:outline-none focus:border-emerald-600">
                                            ...
                                            @foreach($items as $it)
                                                <option value="{{ $it->id }}" 
                                                    ...
                                                    {{ $it->name }} &bull; {{ $currencySymbol }}{{ number_format($it->purchase_price ?? 0, 2) }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <input type="text" name="items[{{ $index }}][item_name]" list="billCatalogItemsList" required value="{{ $item->name }}" placeholder="Expense description"
                                ...
                                <td class="py-2.5 px-3">
                                    <div class="space-y-1.5">
                                        <select onchange="onCatalogItemSelect(this)" class="catalog-select w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-slate-700 text-xs font-medium focus:outline-none focus:border-emerald-600">
                                            ...
                                            @foreach($items as $it)
                                                <option value="{{ $it->id }}" data-name="{{ $it->name }}" data-price="{{ $it->purchase_price ?? 0 }}" data-tax-rate="{{ $it->tax->rate ?? 0 }}">
                                                    {{ $it->name }} &bull; {{ $currencySymbol }}{{ number_format($it->purchase_price ?? 0, 2) }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <input type="text" name="items[0][item_name]" list="billCatalogItemsList" required placeholder="Expense description"
                                            oninput="onItemNameInput(this)"
                                            class="item-name-input w-full bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 text-slate-900 text-xs focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 shadow-sm">
                                    </div>
                                </td>
                ...
                <div class="w-72 space-y-2 text-xs">
                    ...
                    @if($bill->paid_amount > 0)
                        <div class="flex justify-between text-xs text-slate-500 pt-1">
                            ...
                            <span class="text-emerald-600 font-semibold">{{ $currencySymbol }}{{ number_format($bill->paid_amount, 2) }}</span>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="glass-card p-6 rounded-2xl border border-slate-200 shadow-sm bg-white space-y-4">
            <div>
                <label class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2">Internal Notes / Payment Reference</label>
                <textarea name="notes" rows="2" placeholder="Reference notes for internal accounts..."
                    class="w-full bg-white border border-slate-300 rounded-xl p-3 text-slate-900 text-xs focus:outline-none focus:border-emerald-600 shadow-sm">{{ old('notes', $bill->notes) }}</textarea>
            </div>
        </div>

        <!-- Submission Buttons -->
        <div class="flex items-center justify-end gap-4">
            <a href="{{ route('bills.show', $bill->id) }}" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-xl transition">Cancel</a>
            <button type="submit" class="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-md shadow-emerald-600/20 transition flex items-center gap-2">
                ...
                <span class="text-white force-white">Update Bill</span>
            </button>
        </div>
    </form>
</div>
...
@section('scripts')
<script>
    let rowIndex = {{ count($bill->items) > 0 ? count($bill->items) : 1 }};
    ...
    function onItemNameInput(inputElem) {
        ...
        if (matchedItem) {
            ...
            if (priceInput) priceInput.value = parseFloat(matchedItem.purchase_price || 0).toFixed(2);
    ...
    function addRow() {
        ...
        catalogItems.forEach(it => {
            catalogOptions += `<option value="${it.id}" data-name="${it.name}" data-price="${it.purchase_price || 0}" data-tax-rate="${it.tax ? it.tax.rate : 0}">${it.name} &bull; ${currencySymbol}${parseFloat(it.purchase_price || 0).toFixed(2)}</option>`;
        });
        ...
        tr.innerHTML = `
            <td class="py-2.5 px-3">
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="items[${rowIndex}][item_name]" list="billCatalogItemsList" required placeholder="Expense description"
                        oninput="onItemNameInput(this)"
                        class="item-name-input w-full bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 text-slate-900 text-xs focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 shadow-sm">
                </div>
            </td>
