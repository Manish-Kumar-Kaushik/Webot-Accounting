...
<div class="max-w-5xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-slate-900 tracking-tight">Create New Invoice</h1>
            <p class="text-xs text-slate-500">Issue an itemized customer invoice with catalog products, services, and taxes</p>
        </div>
        <a href="{{ route('invoices.index') }}" class="text-xs text-slate-500 hover:text-slate-800">&larr; Back to Invoices</a>
    </div>
    ...
    <form action="{{ route('invoices.store') }}" method="POST" id="invoiceForm" class="space-y-6">
        ...
        <div class="glass-card p-6 rounded-2xl border border-slate-200 shadow-sm bg-white space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="invoice_number" value="{{ old('invoice_number', $nextInvoiceNumber) }}" required
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2">Order / PO Reference</label>
                    <input type="text" name="order_number" value="{{ old('order_number') }}" placeholder="e.g. PO-9821"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 pt-3 border-t border-slate-100">
                ...
                <div>
                    ...
                    <input type="date" name="due_date" value="{{ old('due_date', date('Y-m-d', strtotime('+15 days'))) }}" required
                        class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-slate-900 text-xs focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 shadow-sm">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2">Revenue Category</label>
                    <select name="category_id" class="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-slate-900 text-xs focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 shadow-sm">
        ...
        <div class="glass-card p-6 rounded-2xl border border-slate-200 shadow-sm bg-white space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-[11px] text-slate-500">Pick catalog items or type custom services with real-time totals</p>
                </div>
                        ...
                        <tr class="item-row">
                            <td class="py-2.5 px-3">
                                <div class="space-y-1.5">
                                    ...
                                    <!-- Custom or Edited Item Name / Description Input with Datalist -->
        ...
        <div class="flex items-center justify-end gap-4">
            <a href="{{ route('invoices.index') }}" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-xl transition">Cancel</a>
            <button type="submit" class="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-md shadow-emerald-600/20 transition flex items-center gap-2">
                ...
                <span class="text-white force-white">Issue & Save Invoice</span>
            </button>
        </div>
    </form>
</div>
...
<script>
    ...
    async function handleQuickCustomerSubmit(e) {
        ...
        const payload = {
            ...
            tax_number: document.getElementById('qc_tax_number').value,
            address: document.getElementById('qc_address').value,
            city: document.getElementById('qc_city').value,
            country: document.getElementById('qc_country').value,
        };
